...
function addRouteTableBlock(prefill = false, customRows = null) {
  ...
  if (customRows) {
    tbody.innerHTML = customRows.map((r, i) => renderRouteRow([
      i + 1, r.lease || '', r.route || '', r.tippersLease || r.tpLease || '', r.tippersAll || r.tpAll || '', r.length || '', r.roadType || 'Unpaved', r.recom || 'Unpaved', r.constructedBy || r.constr || 'Lease Owner', r.map || 'Route Map attached'
    ])).join("");
  } else if (prefill) {
...
function addClusterTableBlock(prefill = false, customRows = null) {
  ...
  if (customRows) {
    tbody.innerHTML = customRows.map(r => renderClusterRow([
      r.cluster || '', r.route || '', r.tippersCluster || r.tpCluster || '', r.tippersAll || r.tpAll || '', r.length || '', r.roadType || 'Unpaved', r.recom || 'Unpaved', r.constructedBy || r.constr || 'Lease Owner', r.map || 'Route Map attached'
    ])).join("");
  } else if (prefill) {
...
function initRoutesTable() {
  ...
  if (!container) return;
  container.innerHTML = "";
  if (S.activeProject && S.activeProject.anx4Data && S.activeProject.anx4Data.individual && S.activeProject.anx4Data.individual.length > 0) {
    addRouteTableBlock(false, S.activeProject.anx4Data.individual);
  } else {
...
function initClustersTable() {
  ...
  if (!container) return;
  container.innerHTML = "";
  if (S.activeProject && S.activeProject.anx4Data && S.activeProject.anx4Data.cluster && S.activeProject.anx4Data.cluster.length > 0) {
    addClusterTableBlock(false, S.activeProject.anx4Data.cluster);
  } else {
