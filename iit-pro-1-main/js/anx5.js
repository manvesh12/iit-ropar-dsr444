...
function renderAnx5SavedTables() {
  if (!S.activeProject || !S.activeProject.anx5Data) return;
  const data = S.activeProject.anx5Data;

  if (data.mining) {
    const tbody = document.querySelector('#anx5-mining tbody');
    if (tbody) {
      tbody.innerHTML = '';
      data.mining.forEach((row, i) => {
        let area = parseFloat(row.area) || 0;
        let bulkDensity = parseFloat(row.bulkDensity) || 1.54;
        let depth = parseFloat(row.depth) || 1.74;
        let gross = area * 10000 * depth * bulkDensity;
        let net = gross * 0.60;
        let epSelect = `<select><option>Existing</option><option>Proposed</option></select>`;
        addRowAnx5('anx5-mining', [
          i + 1, row.river, row.barCode, row.details, area.toString(), row.lat, row.lng, 'NA', 'More than 500m', 'Yes', bulkDensity.toString(), depth.toString(), gross.toFixed(2), net.toFixed(2), 'Sand', epSelect, 'EC Granted', ''
        ]);
      });
    }
  }
  ...
  if (data.patta) {
    const tbody = document.querySelector('#anx5-patta tbody');
    if (tbody) {
      tbody.innerHTML = '';
      data.patta.forEach((row, i) => {
        let area = parseFloat(row.area) || 0;
        let reserve = Math.round(area * 10000 * 3 * 1.52);
        let mineral = Math.round(reserve * 0.60);
        let epSelect = `<select><option>Existing</option><option>Proposed</option></select>`;
        addRowAnx5('anx5-patta', [
          i + 1, row.owner, row.khasra, area.toString(), row.lat, row.lng, 'Jalandhar', row.tehsil, row.village, reserve.toString(), mineral.toString(), epSelect, 'EC Granted', ''
        ]);
      });
    }
  }
  ...
  if (data.desilt) {
    const tbody = document.querySelector('#anx5-desilt tbody');
    if (tbody) {
      tbody.innerHTML = '';
      data.desilt.forEach((row, i) => {
        let epSelect = `<select><option>Existing</option><option>Proposed</option></select>`;
        addRowAnx5('anx5-desilt', [
          row.dam, row.controlledBy, row.lat, row.lng, 'Jalandhar', 'Phillaur', 'Powari', row.size, row.qty, epSelect, ''
        ]);
      });
    }
  }
  ...
  if (data.msand) {
    const tbody = document.querySelector('#anx5-msand tbody');
    if (tbody) {
      tbody.innerHTML = '';
      data.msand.forEach((row, i) => {
        let epSelect = `<select><option>Existing</option><option>Proposed</option></select>`;
        addRowAnx5('anx5-msand', [
          row.name, row.owner, 'Jalandhar', row.tehsil, row.village, row.geo, row.qty, epSelect, ''
        ]);
      });
    }
  }
...
window.renderAnx5SavedTables = renderAnx5SavedTables;
