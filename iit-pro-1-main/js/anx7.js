  ...
  if (rows) {
    data = rows.map((r, i) => {
      ...
      return [
        ...
        r.lease || '',
        r.route || '',
        r.tippersLease || r.tpLease || '',
        r.tippersAll || r.tpAll || '',
        r.length || '',
        r.roadType || 'Unpaved',
        r.recom || 'Unpaved',
        r.constructedBy || r.constr || 'Lease Owner',
        r.map || 'Route Map attached'
      ];
    });
  } else {
    data = prefill ? ANX7_DEFAULT_ROUTES.map((row, i) => [i + 1, ...row]) : [[1, 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA']];
  }
  tbody.innerHTML = data.map(row => renderRouteRowAnx7(row)).join('');
...
function addClusterTableBlockAnx7(prefill = false, rows = null, title = '') {
  ...
  if (rows) {
    ...
    data = prefill ? ANX7_DEFAULT_CLUSTERS : [['NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA', 'NA']];
  }
...
function initAnx7() {
  ...
  if (!individual && !cluster) return;

  if (individual) {
    individual.innerHTML = '';
    if (S.activeProject && S.activeProject.anx7Data && S.activeProject.anx7Data.individual && S.activeProject.anx7Data.individual.length > 0) {
      addRouteTableBlockAnx7(false, S.activeProject.anx7Data.individual);
    } else {
      addRouteTableBlockAnx7(true);
    }
  }

  if (cluster) {
    cluster.innerHTML = '';
    if (S.activeProject && S.activeProject.anx7Data && S.activeProject.anx7Data.cluster && S.activeProject.anx7Data.cluster.length > 0) {
      addClusterTableBlockAnx7(false, S.activeProject.anx7Data.cluster);
    } else {
      addClusterTableBlockAnx7(true);
    }
  }
  renderPdfUploadUIAnx7();
}
